<script language="javascript">				
	...
	{				
		...
		window.location ="index.php?com=product&typeparent=<?=@$_GET['typeparent']?>&act=<?php if($_REQUEST['act']=='edit_sub') echo 'edit_sub'; else echo 'add_sub';?><?php if($_REQUEST['id']!='') echo"&id=".$_REQUEST['id']; ?>&id_list="+a.value;	
		return true;
	}
	...
	{				
		...
		window.location ="index.php?com=product&typeparent=<?=@$_GET['typeparent']?>&act=<?php if(@$_REQUEST['act']=='edit_sub') echo 'edit_sub'; else echo 'add_sub';?><?php if(@$_REQUEST['id']!='') echo"&id=".$_REQUEST['id']; ?>&id_list=<?=$_GET['id_list']?>&id_cat="+a.value;	
		return true;
	}	
...
function get_main_cat()
	...
	{
	$sql_huyen="select * from table_product_item where hienthi=1 and com='".$_GET["typeparent"]."' and  id_cat=".@$_REQUEST['id_cat']." order by stt, id asc";
...
<form name="supplier" id="validate" class="form" action="index.php?com=product&act=save_sub&typeparent=<?=@$_GET['typeparent']?>&curPage=<?=$_REQUEST['curPage']?>" method="post" enctype="multipart/form-data">
	<div class="widget">
...
<?php if ($image_active=="on") {?>	
      ...
      <?php if ($_REQUEST['act']=='edit_sub' && $item['thumb']!='')
        {?>
        <img src="<?php if($item['photo']!=NULL) echo _upload_product_sub.$item['photo']; else echo 'images/no_image.jpg';?>"  alt="NO PHOTO" style="max-width:300px;" />
        
         <a class="delete_img_present" title="Xoá ảnh" onclick="Action_Delete_Img('index.php?com=product&act=save_sub&typeparent=<?=$_GET['typeparent']?>&id=<?=@$item['id']?>&delete_img_present=delete_img');return false;">Xoá ảnh</a>
  ...
  <div class="tab_gaconit">      
	...
	<input type="hidden" name="referer_link" id="id" value="index.php?com=product&act=man_sub&typeparent=<?=$_GET['typeparent']?>&curPage=<?=$_REQUEST['curPage']?>" />
	<input type="submit" value="Lưu" class="blueB" />
	<input type="button" value="Thoát" onclick="javascript:window.location='index.php?com=product&act=man_sub&curPage=<?=$_REQUEST['curPage']?>'" class="blueB" />
