<script language="javascript">				
	...
	{				
		...
		window.location ="index.php?com=news&typeparent=<?=$_GET['typeparent']?>&act=<?php if($_REQUEST['act']=='edit_item') echo 'edit_item'; else echo 'add_item';?><?php if($_REQUEST['id']!='') echo"&id=".$_REQUEST['id']; ?>&id_list="+a.value;	
		return true;
	}
...
<div class="control_frm" style="margin-top:25px;">
    <div class="bc">
        <ul id="breadcrumbs" class="breadcrumbs">
        	            <li><a href="index.php?com=news&act=man_item&typeparent=<?=$_GET['typeparent']?>"><span><?=$name_cap?></span></a></li>
...
<form name="supplier" id="validate" class="form" action="index.php?com=news&act=save_item&typeparent=<?=$_GET['typeparent']?>" method="post" enctype="multipart/form-data">
	<div class="widget">
   ...
   <?php if ($image_active=="on") {?>      
     <div class="formRow" >
      ...
      <?php if ($_REQUEST['act']=='edit_item' && $item['thumb']!='')
        ...
        <img src="<?php if($item['photo']!=NULL) echo _upload_news.$item['photo']; else echo 'images/no_image.jpg';?>"  alt="NO PHOTO" style="max-width:300px;" />
         ...
         <a class="delete_img_present" title="Xoá ảnh" onclick="Action_Delete_Img('index.php?com=news&act=save_item&typeparent=<?=$_GET['typeparent']?>&id=<?=@$item['id']?>&delete_img_present=delete_img');return false;">Xoá ảnh</a>
	...
	<input type="button" value="Thoát" onclick="javascript:window.location='index.php?com=news&act=man_item&typeparent=<?=$_GET['typeparent']?>&curPage=<?=$_REQUEST['curPage']?>'" class="blueB" />
