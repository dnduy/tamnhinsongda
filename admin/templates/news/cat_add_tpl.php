<script language="javascript">	
	...
	{				
		...
		window.location ="index.php?com=news&typeparent=<?=$_GET['typeparent']?>&act=<?php if($_REQUEST['act']=='edit_cat') echo 'edit_cat'; else echo 'add_cat';?><?php if($_REQUEST['id']!='') echo"&id=".$_REQUEST['id']; ?>&id_list="+a.value;	
		return true;
	}
...
function get_main_category()
	{
		$sql="select * from table_news_list where com='$_GET[typeparent]'  order by stt,id";
...
<div class="control_frm" style="margin-top:25px;">
    <div class="bc">
        <ul id="breadcrumbs" class="breadcrumbs">
        	 ...
        	 <li><a href="index.php?com=news&act=man_cat&typeparent=<?=$_GET['typeparent']?>"><span><?=$name_cap?></span></a></li>
...
<form name="supplier" id="validate" class="form" action="index.php?com=news&act=save_cat&typeparent=<?=$_GET['typeparent']?>" method="post" enctype="multipart/form-data">
	<div class="widget">
     ...
     <div class="formRow">
			...
			<div class="formRight">
                    ...
                    <img src="<?php if($item['photo']!=NULL) echo _upload_news.$item['thumb']; else echo 'images/no_image.jpg';?>"  alt="NO PHOTO" style="max-width:300px;" />
                    <a class="delete_img_present" title="Xoá ảnh" onclick="Action_Delete_Img('index.php?com=news&act=save_cat&typeparent=<?=$_GET['typeparent']?>&id=<?=@$item['id']?>&delete_img_present=delete_img');return false;" >Xoá ảnh</a>
                    <br>
                    <?php }?>
	...
	<input type="hidden" name="referer_link" id="id" value="index.php?com=news&act=man_cat&typeparent=<?=$_GET['typeparent']?>" />
	<input type="submit" value="Lưu" class="blueB" />
	<input type="button" value="Thoát" onclick="javascript:window.location='index.php?com=news&act=man_cat&typeparent=<?=$_GET['typeparent']?>&curPage=<?=$_REQUEST['curPage']?>'" class="blueB" />
