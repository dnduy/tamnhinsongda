...
$(document).ready(function() { 
	...
	//Load combobox quốc gia tinh thanh - quan huyen
 ...
 <form action="" class="form-horizontal" id="frm_taodiachi" method="post" name="frm_taodiachi" enctype="multipart/form-data">
  <input name="emailtk"  type="hidden" value="<?=$item_user['email']?>" id="emailtk" >    
     <table class="adminform" style="background-color: white; border: 0; width: 100%;">
       ...
       <input name="hoten" type="text" maxlength="50" id="hoten" value="<?=$item_info["hoten"]?>" class="keycode" style="width:50%;">
       ...
       <input name="diachigiaohang" type="text"  maxlength="50" id="diachigiaohang" value="<?=$item_info["diachigiaohang"]?>" class="keycode" style="width:50%;">
                ...
                <select name="quocgia" id="quocgia">
                    ...
                    <?php 
                        ...
                        for($i=0,$count_tinh=count($quocgia);$i<$count_tinh;$i++) { ?>
                         <option <?php if($item_info['id_quocgia']==$quocgia[$i]['id']) echo 'selected="selected"'; ?> value="<?=$quocgia[$i]['id']?>"><?=$quocgia[$i]["ten_$lang"]?></option>
                    <? }?>
                </select>
                ...
                <select name="tinh" id="tinh">
                    ...
                    <?php 
                        //Load tinh thanh
                        $d->reset();
                        $sql="select id,ten_$lang from #_city_list where hienthi =1 and id_danhmuc=$item_info[id_quocgia] order by stt asc";
                        $d->query($sql);
                        $tinhthanh = $d->result_array();
                        for($i=0,$count_tinh=count($tinhthanh);$i<$count_tinh;$i++) { ?>
                         <option <?php if($item_info['id_tinh']==$tinhthanh[$i]['id']) echo 'selected="selected"'; ?> value="<?=$tinhthanh[$i]['id']?>"><?=$tinhthanh[$i]["ten_$lang"]?></option>
                    <? }?>
                </select>
                 		...
                 		<tr>
			    			...
			    			<div class="advance-control">
			                     <?php
			                     $d->reset();
		                        $sql="select id,ten_$lang from #_city_cat where hienthi =1 and id_list=$item_info[id_tinh] order by stt asc";
		                        $d->query($sql);
			                  ...
			                  <select name="huyen" id="huyen">
			                    ...
			                    <?php 
									...
									if (($item_info["id_huyen"])>0) { 
				                        for($i=0,$count_huyen=count($huyen);$i<$count_huyen;$i++) { ?>
				                         <option <?php if($item_info['id_huyen']==$huyen[$i]['id']) echo 'selected="selected"'; ?> value="<?=$huyen[$i]['id']?>"><?=$huyen[$i]["ten_$lang"]?></option>
				                    <? } } else {?>
			    			...
			    			<div class="advance-control">
			                  ...
			                  <select name="phuong" id="phuong">
			                    ...
			                    <?php 
			                        //Load tinh thanh
			                        $d->reset();
			                        $sql="select id,ten_$lang from #_city_item where hienthi =1 and id_cat=$item_info[id_huyen] order by stt asc";
			                        $d->query($sql);
			                        $phuong = $d->result_array();
									if (($item_info["id_phuong"])>0) { 
			                        for($i=0,$count_huyen=count($phuong);$i<$count_huyen;$i++) { ?>
			                         <option <?php if($item_info['id_phuong']==$phuong[$i]['id']) echo 'selected="selected"'; ?> value="<?=$phuong[$i]['id']?>"><?=$phuong[$i]["ten_$lang"]?></option>
			                    <? } } else {?>
