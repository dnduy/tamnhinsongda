...
$(document).ready(function() { 
	...
	$("#huyen").change(function(){
	 ...
	 $('#Send_createaddress').click(function() {  
			...
			$.ajax({
				...
				success:function(response){
					...
					if(response==1){//them thanh cong
						$("#after_submit").html('');
						$("#Send_createaddress").after(function() {
                            $(".customNotify").jmNotify({
								html: '<?=_taodiachimoithanhcong?>',
								delay:1500,
								onClose:redirect()
							});	
                        });
					...
					else if(response==4){ // trung dia chi giao hang 
						$("#after_submit").html('');
						$("#Send_createaddress").after(function() {
                            $(".customNotify").jmNotify({
								html: '<?=_diachigiaohangerror?>'
							});	
                        });
...
<div class="bold-title"><?=_taodiachi?></div><!--end bold-title-->
 ...
 <form action="" class="form-horizontal" id="frm_taodiachi" method="post" name="frm_taodiachi" enctype="multipart/form-data">
  <input name="emailtk"  type="hidden" value="<?=$item_user['email']?>" id="emailtk" >    
     <table class="adminform" style="background-color: white; border: 0; width: 100%;">
       ...
       <input name="hoten" type="text" maxlength="50" id="hoten" class="keycode" style="width:50%;">
       ...
       <input name="dienthoai" type="text"  maxlength="50" id="dienthoai" class="keycode" style="width:50%;">
       ...
       <input name="diachigiaohang" type="text"  maxlength="50" id="diachigiaohang" class="keycode" style="width:50%;">
                ...
                <select name="quocgia" id="quocgia">
                    <option value=""><?=_chonquocgia?></option>
                    <?php 
                        //Load tinh thanh
                        $d->reset();
    			...
    			<div class="advance-control">
                  ...
                  <select name="huyen" id="huyen">
                    ...
                    <?php 
						...
						if (($item_user["huyen"])>0) { 
                        for($i=0,$count_huyen=count($huyen);$i<$count_huyen;$i++) { ?>
                         <option value="<?=$huyen[$i]['id']?>"><?=$huyen[$i]["ten_$lang"]?></option>
                    <? } } else {?>
    			...
    			<div class="advance-control">
                  ...
                  <select name="phuong" id="phuong">
                    ...
                    <?php 
                        //Load tinh thanh
                        $d->reset();
                        $sql="select id,ten_$lang from #_city_item where hienthi =1 order by stt asc";
                        $d->query($sql);
                        $phuong = $d->result_array();
						if (($item_user["phuongxa"])>0) { 
                        for($i=0,$count_huyen=count($phuong);$i<$count_phuong;$i++) { ?>
                         <option value="<?=$phuong[$i]['id']?>"><?=$phuong[$i]["ten_$lang"]?></option>
                    <? } } else {?>
          ...
          <label for="captcha-code" class="control-label"><span class="required">*</span><?=_mabaove?>: </label><!--end captcha-code--> 
            ...
            <tr>
                <td class="aligncenter">
                    <input type="submit" value="<?=_luu?>"  id="Send_createaddress" class="button-blue">
                </td>
            </tr>
        </tbody></table>
