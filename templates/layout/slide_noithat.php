<?php
	...
	$d->reset();
	$sql = "select photo,link, ten_$lang as ten, mota_$lang as mota from #_image_url where hienthi=1 and com='slide_nt' order by stt,id desc";
	$d->query($sql);
...
<div  data-src="thumb/1366x500/1/<?=_upload_hinhanh_l.$slider[0]["photo"]?>" data-link="<?=$slider[0]["link"]?>" data-title="<?=$slider[0]["ten_$lang"]?>" data-target="_blank">
		</div>

	<?php for ($i=0;$i<count($slider);$i++) {?>	 		
		<div  data-src="thumb/1366x500/1/<?=_upload_hinhanh_l.$slider[$i]["photo"]?>" data-link="<?=$slider[$i]["link"]?>" data-title="<?=$slider[$i]["ten_$lang"]?>" data-target="_blank">
		</div>
     <?php }?>   
