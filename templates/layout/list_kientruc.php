<?php
	...
	$d->reset();
	$sql = "select photo,link, ten_$lang as ten from #_image_url where hienthi=1 and com='slide_kientruc' order by stt,id desc";
	$d->query($sql);
	$slide_project=$d->result_array();
	

	$d->reset();
	$sql = "select id, ten_$lang as ten,tenkhongdau_$lang as tenkhongdau,mota_$lang as mota,icon,photo from #_product_list where hienthi=1 and com='kientruc' order by stt asc";
	$d->query($sql);
  ...
  $d->reset();
  $sql = "select id,ten_$lang as ten,tenkhongdau_$lang as tenkhongdau,mota_$lang as mota,photo from #_news where hienthi=1 and com='taisaochonchungtoi_kt' order by id desc";
  $d->query($sql);
  $taisaochonchungtoi=$d->result_array();
  
  
  	$d->reset();
	$sql_banner_giua = "select banner_$lang as banner from #_banner where com='banner_chungtoi_kt' ";
...
<script type="text/javascript">
  ...
  responsive: [
	...
	$(".list_catalog_group .box_list_catalog_group").slick({
...
<div class="container">
	...
	<div class="box_list_group_catalog">
		...
		<?php /*?>	<li id="tab_pro_indexformat"><a href="#tab-pro-all"><?=_tatca?></a></li><?php */?>
			...
			<?php $d->reset();
			$sql_news = "select ten_$lang as ten,id,tenkhongdau_$lang as tenkhongdau,photo,alt_$lang as alt,mota_$lang as mota from #_product where hienthi=1 and com='kientruc'  order by id desc limit 8";
			$d->query($sql_news);
...
<?php */?>
		 ...
		 <?php foreach ($project_list as $l =>$v_list) {?>
			...
			<?php $d->reset();
			$sql_news = "select ten_$lang as ten,id,tenkhongdau_$lang as tenkhongdau,photo,alt_$lang as alt,mota_$lang as mota from #_product where hienthi=1 and com='kientruc'  and id_list='".$v_list["id"]."'   order by id desc limit 8";
			$d->query($sql_news);
			$pro_moi = $d->result_array();
			if (count($pro_moi)>0){?>
		   ...
		   <div class="product-item col-lg-3 col-md-3 col-sm-6 col-xs-12" >
			...
			<div class="portfolio-item">
				...
				<div class="product-image">
					<a href="<?=$com?>/<?=$index_sp["tenkhongdau"]?>-<?=$index_sp["id"]?>.html" title="<?=$index_sp["ten"]?>">
					<img  effect="mono" inverse="true" class="img-responsive has-tt colorup colorUpped" src="thumb/380x380/1/<?php if($index_sp['photo']!=NULL) echo _upload_product_l.$index_sp['photo']; else echo 'images/no-image-available.png';?>" alt="<?=$index_sp["alt"]?>" />
					</a>
				</div><!--end product-image-->
				...
				<div class="product-info portfolio-item-overlay">
				 ...
				 <div class="product-name">
					...
					<h3> <a href="<?=$com?>/<?=$index_sp["tenkhongdau"]?>-<?=$index_sp["id"]?>.html" title="<?=$index_sp["ten"]?>"><?=catchuoi($index_sp["ten"],40)?></a></h3>
